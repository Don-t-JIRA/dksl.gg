-- Tendency
INSERT INTO `tendency`(`id`, `name`, `initial`) VALUES ('challenging', '#도전적인', 'C');
INSERT INTO `tendency`(`id`, `name`, `initial`) VALUES ('stable', '#안정적인', 'S');
INSERT INTO `tendency`(`id`, `name`, `initial`) VALUES ('vintage', '#올드한', 'V');
INSERT INTO `tendency`(`id`, `name`, `initial`) VALUES ('mz', '#MZ한', 'M');
INSERT INTO `tendency`(`id`, `name`, `initial`) VALUES ('early', '#초반형', 'E');
INSERT INTO `tendency`(`id`, `name`, `initial`) VALUES ('lately', '#후반형', 'L');
INSERT INTO `tendency`(`id`, `name`, `initial`) VALUES ('detector', '#철거반', 'D');
INSERT INTO `tendency`(`id`, `name`, `initial`) VALUES ('killer', '#장의사', 'K');

-- Tier
INSERT INTO `tier`(`id`, `name`) VALUES ('iron', '아이언');
INSERT INTO `tier`(`id`, `name`) VALUES ('bronze', '브론즈');
INSERT INTO `tier`(`id`, `name`) VALUES ('silver', '실버');
INSERT INTO `tier`(`id`, `name`) VALUES ('gold', '골드');
INSERT INTO `tier`(`id`, `name`) VALUES ('platinum', '플래티넘');
INSERT INTO `tier`(`id`, `name`) VALUES ('emerald', '에메랄드');
INSERT INTO `tier`(`id`, `name`) VALUES ('diamond', '다이아몬드');
INSERT INTO `tier`(`id`, `name`) VALUES ('master', '마스터');
INSERT INTO `tier`(`id`, `name`) VALUES ('grandmaster', '그랜드마스터');
INSERT INTO `tier`(`id`, `name`) VALUES ('challengerr', '챌린저');

-- Temp User
INSERT INTO `team`(`name`) VALUES ('SSAFY');